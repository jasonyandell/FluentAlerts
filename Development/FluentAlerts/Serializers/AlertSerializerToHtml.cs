        ...
        private void AppendSerialization(IList<object> values, int columns, string firstColumnBackground, string otherColumnsBackground, StringBuilder sb)
        {
            ...
            for (var col = 0; col <= (columns - 1); col++)
            {
                ...
                if (col < values.Count)
                {
                    ...
                    if (value is CompositeFluentAlert)
                    {
                        AppendSerialization((CompositeFluentAlert)value, sb);
                    }
                    else if (value is FluentAlertTable)
                    {
                        AppendSerialization((FluentAlertTable)value, sb);
                    }
                    else
                    {
                        sb.Append(value);
